...
2. Перенести в таргет StorageService файлы StorageService и Post. Эти файлы не должны быть добавлены в таргет основгого приложения.
3. Добавить с проект необходимые модификаторы доступа `public` и импорты фреймфорка StorageService для успешного запуска проекта

## Задача 2
1. Создать дубликат текущей схемы проекта, настроить Run конфигурацию с Release конфигурацией
2. На экране `ProfileViewController` настроить разный цвет бекграунда для Debug и Release сборки с помощью флага компиляции DEBUG
3. Запустить обе схемы и проверить, что цвет бекграунда меняется.
